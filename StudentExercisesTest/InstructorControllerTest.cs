...
namespace StudentExercisesTest
{
    public class InstructorTestController
    {
        ...
        private Instructor dummyInstructor { get; } = new Instructor
        {
            FirstName = "Lance",
            LastName = "Penn",
        ...
        private string url { get; } = "/api/instructor";


        // Reusable method to create a new coffee in the database and return it
        public async Task<Instructor> CreateDummyInstructor()
        {
            ...
            using (var client = new APIClientProvider().Client)
            {
                ...
                string lanceAsJSON = JsonConvert.SerializeObject(dummyInstructor);
                ...
                Instructor newlyCreatedInstructor = JsonConvert.DeserializeObject<Instructor>(responseBody);

                return newlyCreatedInstructor;
            }
        }

        // Reusable method to deelte a coffee from the database
        public async Task deleteDummyInstructor(Instructor instructorToDelete)
        {
            using (HttpClient client = new APIClientProvider().Client)
            {
                HttpResponseMessage deleteResponse = await client.DeleteAsync($"{url}/{instructorToDelete.Id}");
        ...
        [Fact]
        public async Task Create_Instructor()
        {
            using (var client = new APIClientProvider().Client)
            {
                ...
                Instructor newInstructor = JsonConvert.DeserializeObject<Instructor>(responseBody);

                // Make sure it's really there
                Assert.Equal(HttpStatusCode.OK, response.StatusCode);
                Assert.Equal(dummyInstructor.LastName, newInstructor.LastName);
                Assert.Equal(dummyInstructor.FirstName, newInstructor.FirstName);

                // Clean up after ourselves
                await deleteDummyInstructor(newInstructor);
        ...
        public async Task Delete_Instructor()
        {
            ...
            // Delete it
            await deleteDummyInstructor(newLancePenn);
        ...
        [Fact]
        public async Task Get_All_Instructor()
        {
            ...
            using (var client = new APIClientProvider().Client)
            {
                ...
                List<Instructor> instructors = JsonConvert.DeserializeObject<List<Instructor>>(responseBody);
        ...
        [Fact]
        public async Task Get_Single_Instructor()
        {
            using (HttpClient client = new APIClientProvider().Client)
            {
                ...
                // Turn the JSON into C#
                Instructor LancePennFromDB = JsonConvert.DeserializeObject<Instructor>(responseBody);

                // Did we get back what we expected to get back? 
                Assert.Equal(HttpStatusCode.OK, response.StatusCode);
                Assert.Equal(dummyInstructor.LastName, LancePennFromDB.LastName);
                Assert.Equal(dummyInstructor.FirstName, LancePennFromDB.FirstName);

                // Clean up after ourselves-- delete the dummy coffee we just created
                await deleteDummyInstructor(LancePennFromDB);
